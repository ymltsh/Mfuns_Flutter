...
import 'article_text_copy_page.dart';
import 'rich_content_normalizer.dart';

export 'rich_content_normalizer.dart'
    show normalizeRichContent, richContentToPlainText;

/// 桌面端（鼠标/触控板）启用 SelectionArea：拖拽即选区，滚动靠滚轮，
/// 与列表滚动无手势冲突。
bool _useSelectionArea(BuildContext context) =>
    kIsWeb ||
...
class RichContentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _buildContent(context, markdown: markdown);
  }

  Widget _buildContent(BuildContext context, {required String markdown}) {
    final colors = Theme.of(context).colorScheme;
    final styleSheet = MarkdownStyleSheet.fromTheme(Theme.of(context)).copyWith(
      p: Theme.of(
        context,
      ).textTheme.bodyLarge?.copyWith(height: 1.7, color: colors.onSurface),
    ...
    // Markdown 自身保持不可选择，避免多个 SelectableText 与文章列表争抢
    // 拖动手势。桌面端由一个 SelectionArea 统一处理跨段落选择；移动端
    // 长按后进入独立复制页。
    final body = MarkdownBody(
      data: markdown,
      selectable: false,
      builders: {'pre': _CodeBlockBuilder(styleSheet: styleSheet)},
    ...
    final content = _useSelectionArea(context)
        ? SelectionArea(child: body)
        : GestureDetector(
            behavior: HitTestBehavior.translucent,
            onLongPressStart: (details) =>
                _showMobileCopyMenu(context, markdown, details.globalPosition),
            child: body,
          );
    return Card(
      elevation: 0,
      ...
      child: Padding(
        padding: padding,
        child: markdown.isEmpty ? const Text('正文暂无可展示内容') : content,
      ),
    );
  }

  Future<void> _showMobileCopyMenu(
    BuildContext context,
    String markdown,
    Offset globalPosition,
  ) async {
    final overlay = Overlay.of(context).context.findRenderObject() as RenderBox;
    final action = await showMenu<_MobileCopyAction>(
      context: context,
    ...
    if (action != _MobileCopyAction.open || !context.mounted) return;
    final text = richContentToPlainText(markdown);
    await Navigator.of(context).push(
      MaterialPageRoute<void>(builder: (_) => ArticleTextCopyPage(text: text)),
    );
  }
}

enum _MobileCopyAction { open }
    ...
    final image = ClipRRect(
      ...
      child: Image.network(
        ...
        errorBuilder: (_, __, ___) =>
            SizedBox(height: 100, child: Center(child: Text('$alt 加载失败'))),
      ),
    );
    return Padding(
...
class _CodeBlockBuilder extends MarkdownElementBuilder {
  _CodeBlockBuilder({required this.styleSheet});
