...
class ExportStorage {
  ...
  static Future<ExportResult> _persistAndroid(ExportResult result) async {
    ...
    final isVideo = result.mimeType.startsWith('video/');
    final directory = isMarkdown
        ? 'Documents'
        : isVideo
            ? 'Movies'
            : 'Pictures';
  ...
  /// 桌面端：直接复制到用户 Documents / Pictures / Videos 目录。
  static Future<ExportResult> _persistDesktop(ExportResult result) async {
    final isMarkdown = result.mimeType == 'text/markdown';
    final isVideo = result.mimeType.startsWith('video/');
    final base = Directory(
      '${homeDirectory()}${Platform.pathSeparator}'
      '${isMarkdown ? 'Documents' : isVideo ? 'Videos' : 'Pictures'}'
      '${Platform.pathSeparator}$folderName',
    );
