...
import '../user/blacklist_page.dart';
...
const _qualityOptions = <String>['', '360p', '480p', '540p', '720p', '1080p'];
  ...
  Future<void> _checkUpdate() async {
    ...
    try {
      final manifest = await UpdateChecker.fetch(
        acceleratorBase: _acceleratorBase,
      );
      if (!mounted) return;
  ...
  void _showUpToDate(UpdateRelease latest) {
    showDialog<void>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          '当前已是最新版本 v${AppConfig.appVersion}。\n'
          '最新发布：${latest.name}'
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(
                latest.notes.isEmpty ? '暂无更新说明' : latest.notes,
      ...
      children: [
        ...
        const Row(
          ...
          children: [
            ...
            SizedBox(width: 6),
            Flexible(child: Text('如果喜欢这个项目，欢迎在 GitHub 点个 Star 支持我们！')),
          ],
        ),
      ],
    );
                  ...
                  _SettingsSection(
                    ...
                    children: [
                      ...
                      _SettingsTile(
                        ...
                        title: '黑名单',
                        subtitle: '查看和管理已拉黑的用户',
                        onTap: () => Navigator.of(context).push(
                          MaterialPageRoute<void>(
                            builder: (_) =>
                                BlacklistPage(controller: widget.controller),
                          ),
                        ),
                      ),
                      _ProfileEntryLayoutTile(controller: widget.controller),
                      SwitchListTile(
                      ...
                      SwitchListTile(
                        ...
                        onChanged: widget.controller.setBackgroundNotifications,
                      ),
                    ],
                  ),
...
class _PlayerSettingsPageState extends State<PlayerSettingsPage> {
  ...
  double get _sideRatio =>
      UserPreferences.landscapeSideRatios[UserPreferences
              .landscapeSideRatios
              .length -
          1 -
          _sideRatioIndex];
  ...
  Future<void> _load() async {
    ...
    setState(() {
      ...
      final storedRatio = results[3] as double;
      final storedIndex = UserPreferences.landscapeSideRatios.indexWhere(
        (ratio) => (ratio - storedRatio).abs() < .01,
      );
...
class _ArticleReaderSettingsPageState extends State<ArticleReaderSettingsPage> {
  ...
  Future<void> _load() async {
    ...
    setState(() {
      ...
      _fullCommentInput = values[1];
      _showDislike = values[2];
      _loaded = true;
    });
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(
      ...
      children: [
        _SettingsCard(
          children: [
            SwitchListTile(
              ...
              onChanged: _loaded
                  ? (value) {
                      setState(() => _articleToolsEnabled = value);
                      UserPreferences.saveArticleToolsFab(value);
                    }
                  : null,
            ),
            const Divider(height: 1),
            SwitchListTile(
              value: _fullCommentInput,
              title: Text(
                '显示完整评论输入框',
                style: TextStyle(
                  color: AppPalette.of(context).muted,
              ...
              subtitle: Text(
                '开启后文章、视频和动态页始终显示输入框；关闭时收纳为右下角按钮',
                style: TextStyle(
                  color: AppPalette.of(context).muted,
                  fontSize: 12,
                ),
              ...
              onChanged: _loaded
                  ? (value) {
                      setState(() => _fullCommentInput = value);
                      UserPreferences.saveFullCommentInput(value);
                    }
                  : null,
            ),
            const Divider(height: 1),
            SwitchListTile(
              ...
              secondary: CircleAvatar(
                ...
                foregroundColor: AppPalette.of(context).primary,
                child: const Icon(Icons.thumb_down_alt_outlined, size: 20),
              ),
              onChanged: _loaded
                  ? (value) {
        ...
        const SizedBox(height: 10),
        Text(
          '上述选项默认关闭。文章工具按钮可在阅读区域内拖动；评论输入框收纳后可从右下角展开。',
          style: TextStyle(
            color: AppPalette.of(context).muted,
            fontSize: 12,
            height: 1.5,
    ...
    body: ListView(
      ...
      children: [
        ...
        TextField(
          controller: _name,
          maxLength: 16,
          decoration: const InputDecoration(labelText: '昵称', counterText: ''),
        ),
        const SizedBox(height: 12),
