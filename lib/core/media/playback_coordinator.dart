...
class MfunsPlaybackCoordinator {
  ...
  /// 页面重新成为可见路由时，把已有播放器重新登记为全局 owner。
  ///
  /// 与 [bindVideo] 的“新播放器抢占旧播放器”语义不同：这里只恢复
  /// “控制权”，**不自动播放**（配合用户点击播放按钮的 `requestPlay()`）。
  Future<void> claimExistingVideo(
    VideoPlayerController player, {
    String? url,
    int? part,
    String? title,
  ...
  /// 已有播放器重新获得当前 owner（页面 `didPopNext` 时调用）。
  ///
  /// - 只恢复控制权，**不自动播放**：状态按播放器实时状态进入
  ///   foregroundPlaying / foregroundPaused（被其他页面暂停过就是 paused）。
  /// - 若恰有另一个播放器仍占着 owner（如上层路由尚未 dispose），
  ///   回退到 [bindHandle] 语义让其让权。
  @visibleForTesting
  Future<void> claimExistingHandle(
    VideoPlaybackHandle handle, {
    String? url,
    int? part,
    String? title,
    ...
    if (_bound != null && _boundId != handle.id) {
      await bindHandle(
        handle,
        url: url,
        part: part,
    ...
    _machine.claim(videoPlaying: handle.isPlaying);
    PlaybackLog.d(
      'reclaim player ${handle.id} url=$_currentUrl part=$_currentPart '
      'phase=${_machine.phase}',
    );
  }
