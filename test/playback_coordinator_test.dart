...
void main() {
  ...
  group('MfunsPlaybackCoordinator', () {
    ...
    test('reclaim 只恢复控制权，不自动播放', () async {
      final a = FakeVideoHandle('a');
      await coordinator.bindHandle(a);
      ...
      // 模拟 B 抢占后 A 被暂停。
      final b = FakeVideoHandle('b')..playing = true;
      await coordinator.bindHandle(b);
      expect(a.playing, isFalse);
      // B 销毁：unbind 清空 owner。
      expect(coordinator.unbindHandle(b), isTrue);
      expect(coordinator.phase, MfunsPlaybackPhase.stopped);
      a.playCalls = 0;

      // A didPopNext → reclaim：重新成为 owner 但保持暂停。
      await coordinator.claimExistingHandle(
        a,
      ...
      expect(coordinator.phase, MfunsPlaybackPhase.foregroundPaused);
      expect(a.playing, isFalse, reason: 'reclaim 不自动播放');
      expect(a.playCalls, 0, reason: 'reclaim 不应触发 play');

      // 用户点击播放：requestPlay 才能恢复。
      await coordinator.requestPlay();
      expect(a.playing, isTrue);
      expect(coordinator.phase, MfunsPlaybackPhase.foregroundPlaying);
    });

    test('reclaim 恢复被 B 覆盖期间丢失的媒体元数据', () async {
      final a = FakeVideoHandle('a');
      await coordinator.bindHandle(a,
          url: 'https://example.com/a.mp4', part: 1, title: 'A');
      final b = FakeVideoHandle('b');
      await coordinator.bindHandle(b,
          url: 'https://example.com/b.mp4', part: 2, title: 'B');
      expect(coordinator.unbindHandle(b), isTrue);

      await coordinator.claimExistingHandle(a,
          url: 'https://example.com/a.mp4',
          part: 1,
          title: 'A',
          subtitle: 'Mfuns',
          artUri: 'https://example.com/cover.jpg');
      final meta = coordinator.currentMediaMeta;
      expect(meta, isNotNull);
      expect(meta!.title, 'A');
      expect(meta.subtitle, 'Mfuns');
      expect(meta.artUri, 'https://example.com/cover.jpg');
    });

    test('reclaim 时仍存在其他 owner 则让其为 A 让权', () async {
      final a = FakeVideoHandle('a');
      await coordinator.bindHandle(a);
      final b = FakeVideoHandle('b')..playing = true;
      await coordinator.bindHandle(b);

      // B 尚未 dispose 时 A 抢先 reclaim：B 被暂停，A 成为 owner。
      await coordinator.claimExistingHandle(a);
      expect(b.playing, isFalse);
      expect(coordinator.phase, MfunsPlaybackPhase.foregroundPaused);
    });
